...
- 当前项目的节奏由分析节点锁定，不得自行混用：`长镜直出`中普通镜头选择 10 至 15 的整数秒，短反应、插入、转场或瞬时动作可用 5 至 9 秒并填写 `duration_reason`；`短片剪辑`中每行分镜严格选择 2 或 3 秒，`duration_reason` 填“无”。
- 短片剪辑的每行只定义剪辑时间码和镜内内容；连续场景的多行会由系统合并为 4 至 30 秒的 Seedance 2.5 视频片段。每个短镜仍必须写完整的首尾状态、时间轴、切点和资产引用。
...
每个 shot 必须返回核心字段：`shot_id`、`sequence`、`scene_id`、`beat_id`、`duration`、`duration_reason`（10 秒及以上填“无”）、`narrative_purpose`、`emotional_goal`、`shot_size`、`lens`、`perspective`、`camera_angle`、`camera_movement`、`composition`、`blocking`、`character_ids`、`prop_ids`、`action`、`lighting`、`color_tone`、`texture`、`start_frame`、`end_frame`、`transition_in`、`transition_out`、`image_prompt`、`negative_prompt`、`timeline`、`reference_node_ids`。不要输出 `video_prompt` 或 Schema 外字段。

`character_position`、`character_movement`、`eyeline`、`dialogue`、`voiceover`、`sound_effect`、`music_cue`、`previous_shot_id`、`next_shot_id`、`continuity_notes` 和 `continuity_warnings` 不进入严格输出 Schema，系统会填入安全默认值；5 至 9 秒的长镜直出必须在 `duration_reason` 写明原因。`timeline` 每项只返回 `start_second`、`end_second`、`visual_action`、`camera`；系统将表演和声音填为“无”。
