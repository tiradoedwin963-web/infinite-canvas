...
分镜表固定投影为 13 列：镜号、时间码、时长、参考场景图、场景/时间、景别与焦段、机位与运镜、画面构图、角色动作与表演、旁白、环境声与拟声、转场/切点、连续性与生成限制。用可见资产名与图号，不嵌入原图。

锁稿后的视频提示词会按 SD 2.5 的 `4–30` 秒上限、且只在完整镜头边界处建立实际视频段。剧本草案无需预先把镜头拆成旧的 12 秒单元；最终段计划由锁稿后的画布生成并由用户在“调整镜头段”中确认。

没有必要时不强制固定镜头、镜头比例或转场配额；但每镜应清楚交代主体、动作结果、空间方向和首尾状态，避免无依据的复杂动作或运镜。默认只保留旁白、对白、环境声与拟声，不生成 BGM、字幕、Logo 或水印，除非 Brief 已明确确认例外。
