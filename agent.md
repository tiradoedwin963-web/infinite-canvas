...
- 编写图片或视频提示词时自行判断角色是否涉及受保护 IP 或特定商业版本。原创角色和不指向特定商业版本的公版角色保持原写法。
- 存在风险时，节点标题和资产名称可以保留剧本中的称呼，但实际生成使用的 `description`、`prompt`、`image_prompt`、`video_prompt` 和视觉设定不得出现角色、品牌、工作室或具体商业版本名称。
- 当风险来自公版角色的特定商业版本时，上述实际生成字段必须同时去掉商业版本名称和该公版角色名称；不能只删除品牌或工作室名称后继续保留角色名。
- 风险角色改用具体且原创的视觉特征描述，至少覆盖年龄、脸型、发型、体型、服装轮廓与材质、配色、表情、姿态和时代特征。不得使用“某品牌版本”“某工作室风格”或“和某角色一模一样”等表述。
- 风险已经能够通过具体特征改写解决时，必须直接改写并执行用户要求，不得为“是否改成原创近似方案”再次询问用户；只有改写后的要求本身仍存在关键矛盾时才可询问。
